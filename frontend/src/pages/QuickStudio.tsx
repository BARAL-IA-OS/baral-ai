...
export function QuickStudio() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const { data: userData } = await supabase.auth.getUser()
      const userId = userData.user?.id
      if (!userId) throw new Error('No hay una sesión activa para guardar la campaña.')
      ...
      if (saveError) {
        setCampaigns((previous) => [localCampaign, ...previous])
      ...
      setCampaigns((previous) => [savedCampaign, ...previous])
  ...
  function handleDelete(id: string) {
    ...
    setCampaigns((previous) => {
      const next = previous.filter((campaign) => campaign.id !== id)
        ...
        <div className="studio-main">
          ...
          <div className="studio-generator">
            ...
            <div className="studio-prompt-wrap">
              ...
              />
              <button type="button" className={`studio-mic-btn ${listening ? 'is-listening' : ''}`} onClick={handleVoiceInput} aria-label="Dictar campaña con micrófono" title="Dictar campaña">
                {listening ? <MicOff size={17} /> : <Mic size={17} />}
              </button>
            </div>
            <div className="studio-generator-footer">
              <span className="studio-generator-note">La IA generará texto + imagen para cada red.</span>
              <button type="button" className="button button-primary" onClick={() => void handleGenerate()} disabled={!prompt.trim() || generating}>
          ...
          <div className="studio-list">
            <div className="studio-list-head"><span>Mis campañas</span><small>{campaigns.length}</small></div>
            <div className="studio-list-scroll">
              {loadingCampaigns ? (
                ...
                <button key={campaign.id} type="button" className={`studio-card ${campaign.id === selectedId ? 'is-active' : ''}`} onClick={() => setSelectedId(campaign.id)}>
                  <span className="studio-card-icon"><Megaphone size={16} strokeWidth={1.9} /></span>
                  <span className="studio-card-copy"><strong>{campaign.name}</strong><small>{campaign.prompt}</small></span>
                  <span
                    className="studio-card-delete"
                    role="button"
                    tabIndex={0}
                    aria-label="Eliminar campaña"
                    onClick={(event) => { event.stopPropagation(); handleDelete(campaign.id) }}
                    onKeyDown={(event) => { if (event.key === 'Enter') { event.stopPropagation(); handleDelete(campaign.id) } }}
                  >
                    <Trash2 size={15} />
                  </span>
                </button>
